...
아래 영상은 저장소에 함께 들어 있는 합성 Advisory 시나리오만 사용합니다. 고객 데이터나 브로커 연결 없이 조사 흐름을 보여줍니다. 1080p MP4에는 로컬에서 생성한 한국어 내레이션과 번인 자막이 들어 있으며, GIF는 짧은 무음 미리보기입니다.
...
각 분석 화면의 **화면에서 안내 보기**를 누르면 그 화면의 실제 조작 위치를 차례로 강조합니다. 해당 화면에 있을 때는 페이지 직접 이동, 메시지·저널 추가 로딩, 케이스 증거 선택, 메시지 추적 범위와 내보내기 추적 옵션까지 별도 단계로 안내합니다. **장애 조사 튜토리얼**은 저장소에 포함된 합성 스냅샷과 한국어 영상을 사용해 개요, 스냅샷 비교, 저널 보존 탐색, 메시지 추적, 조사 케이스, 증거 내보내기까지 실제 화면을 전환하며 안내합니다.
...
UI를 열면 MQ Watcher는 저장소에 고정된 GitHub Releases API 주소에서 최신 안정 버전 정보를 확인합니다. 이 메타데이터 요청은 user agent에 애플리케이션 버전을 포함하지만 저장소 경로·이름·파일 바이트·캐시된 분석 결과·케이스 메모는 보내지 않습니다. 사용자가 **검증·교체 후 자동 재시작**을 선택하기 전에는 릴리스 파일을 내려받지 않습니다.

자동 교체는 Windows x64 포터블 실행 파일에서만 지원합니다. 업데이트는 현재 실행한 `mq-watcher.exe`를 같은 폴더에서 제자리 교체하며 별도 이름의 새 EXE를 만들지 않습니다. 교체한 뒤 같은 파일을 자동 실행하고 UI가 새 버전 응답을 확인한 후 화면을 다시 불러옵니다. 자동 재연결을 확인하지 못했다면 처음 실행했던 같은 `mq-watcher.exe`를 다시 실행하면 됩니다. 소스, npm, Linux와 미지원 릴리스는 수동 안내 또는 차단 상태로 남습니다. 정확한 지원 범위는 [포터블 배포 및 업데이트 안내](docs/portable-release.md)를 참고하십시오.
...
| 저널 참조 이어보기 | 한 Message ID 추적 |
| --- | --- |
| ![160개 중 150개 참조를 불러온 저널 상세](docs/screenshots/ko/journal-progressive.png) | ![Store와 journal별 메시지 증거](docs/screenshots/ko/message-trace.png) |
