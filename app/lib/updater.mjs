...
function replacementScript({ stagedPath, currentExecutable, expectedSha256, expectedVersion, backupPath, failedPath, processId }) {
  return [
    ...
    "  $restartProcess = Start-Process -FilePath $target -PassThru",
    "  if ($null -eq $restartProcess) { throw 'Replacement executable did not start.' }",
    "  if ($restartProcess.WaitForExit(1000) -and $restartProcess.ExitCode -ne 0) { throw 'Replacement executable failed during restart.' }",
    "}",
    ...
    "  $versionStartInfo.FileName = $target",
    ...
    "  if (-not $versionProcess.Start()) { throw 'Replacement executable version smoke check did not start.' }",
    ...
    "    if (-not (Test-Path -LiteralPath $taskKill)) { $rollbackSafe = $false; throw 'Replacement executable version smoke check timed out and taskkill.exe was unavailable.' }",
    "    & $taskKill /PID $versionProcess.Id /T /F 2>$null | Out-Null",
    "    $versionExited = $versionProcess.WaitForExit(5000)",
    "    $stdoutClosed = $versionStdout.Wait(5000)",
    "    $stderrClosed = $versionStderr.Wait(5000)",
    "    if (-not $versionExited -or -not $stdoutClosed -or -not $stderrClosed) { $rollbackSafe = $false; throw 'Replacement executable version smoke process tree did not stop cleanly.' }",
    "    throw 'Replacement executable version smoke check timed out.'",
    "  }",
    "  $versionProcess.WaitForExit()",
    "  $reportedVersion = ($versionStdout.Result -split \"`r?`n\" | Select-Object -First 1)",
    "  if ($versionProcess.ExitCode -ne 0 -or $null -eq $reportedVersion -or $reportedVersion.Trim() -ne $expectedVersion) { throw (\"Replacement executable version smoke check failed (exit={0}, reported='{1}').\" -f $versionProcess.ExitCode, $reportedVersion) }",
    "  Start-Target",
    "  Remove-Item -LiteralPath $backup -Force -ErrorAction SilentlyContinue",
    "} catch {",
    "  if (-not $rollbackSafe) { throw }",
    ...
    "    $rolledBack = $true",
    "  }",
    "  if ($rolledBack -or (-not $replaced -and (Test-Path -LiteralPath $target))) { Start-Target }",
    "  throw",
    "} finally {",
