...
import { ChevronLeft, ChevronRight, Clock3, FileArchive, Info } from "lucide-react";
...
export function EvidenceTimeline({ result }: { result: ScanResult }) {
  ...
  return <div className="view-stack">
    ...
    <Card className="timeline-card">{visible.length ? <div className="timeline-list">{visible.map((event, index) => { const firstInJournal = index === 0 || visible[index - 1]?.file !== event.file; return <div key={event.id}>{firstInJournal ? <div className="timeline-journal"><FileArchive size={14} /><strong>{event.file}</strong><span>{t("timeline.journalGroup")}</span></div> : null}<div className="timeline-event"><div className="timeline-marker"><span>{event.offset === null ? "—" : formatOffset(event.offset)}</span></div><div className="timeline-content"><div><Badge tone={event.category === "transaction" ? "amber" : event.category === "subscription" ? "violet" : "blue"}>{t(`timeline.category.${event.category}`)}</Badge><strong>{event.command}</strong><Badge tone="neutral">{t(`confidence.${event.confidence}`)}</Badge></div><p>{event.destination} · <code title={event.primaryId}>{compactId(event.primaryId, 28, 12)}</code></p><small>{t("timeline.status", { status: event.status })}</small></div></div></div>; })}</div> : <p className="compare-no-diff">{t("timeline.empty")}</p>}</Card>
