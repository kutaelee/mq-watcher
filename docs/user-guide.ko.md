...
## 실제 화면 주석과 장애 조사 튜토리얼

화면 제목 옆의 **화면에서 안내 보기**를 선택합니다. 나머지 화면을 어둡게 하고 해당 화면의 실제 조작 위치를 직접 강조합니다. 각 주석은 필요한 선행 작업, 사용하면 좋은 상황, 얻을 수 있는 결과와 해석 한계를 함께 설명합니다. 화면에 따라 페이지 직접 이동, 메시지·저널 추가 로딩, 케이스 증거 선택, 메시지 추적 범위와 내보내기 추적 옵션이 별도 단계로 나타납니다.

![메시지 화면의 추가 로딩 상태를 강조하는 화면 안내](screenshots/ko/view-guide.png)

상단의 **장애 조사 튜토리얼**을 선택하면 한국어 영상과 여섯 단계 실습이 열립니다. MQ Watcher가 기준·조사 시점 합성 스냅샷을 불러온 뒤 개요 → 스냅샷 비교 → 저널 보존 탐색 → 메시지 추적 → 조사 케이스 → 증거 번들 내보내기 화면을 실제로 전환합니다. 고객 데이터나 자동 원인 판정은 포함하지 않습니다.
